...
use crate::harness::{
    amp, antigravity, campfire, claude_code, codex, cowork, cursor, dsh, fx, grok, pi,
};
...
pub fn discover_with(mut on_store: impl FnMut(HarnessId, usize)) -> Vec<Session> {
    ...
    on_store(HarnessId::Dsh, out.len());
    scan(HarnessId::Dsh, dsh::DshStore::default_root(), &mut out);
...
impl Session {
    ...
    pub fn read(&self) -> Result<Transcript<Common>> {
        ...
        match (&self.harness, &self.locator) {
            ...
            (HarnessId::Dsh, Locator::Path(p)) => go(dsh::DshStore::default_root(), p),
...
pub fn fingerprints(sessions: &[Session]) -> Vec<String> {
    ...
    for (harness, at) in by_harness {
        ...
        match harness {
            ...
            HarnessId::Dsh => group.files(dsh::DshStore::default_root()),
...
pub fn resume_command(harness: HarnessId, id: &str) -> (String, Vec<String>) {
    ...
    overridden.unwrap_or_else(|| {
        ...
        match harness {
            ...
            HarnessId::Dsh => (
                "dsh".into(),
                vec!["--profile".into(), "tui".into(), "--resume".into(), id],
            ),
