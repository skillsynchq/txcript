...
<sup>6</sup> Grok Bot has no public session-import or resume CLI: sessions convert *from* `grok_bot`, but can't be continued into it. The store still load/saves the on-disk JSONL for conversion and tests.
